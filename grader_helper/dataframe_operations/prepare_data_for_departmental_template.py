...
from . import calculate_total_module_score, make_letter_grade, check_for_weighted_columns, sort_order_columns

def prepare_data_for_departmental_template(df: pd.DataFrame, fail_threshold: int = 35) -> pd.DataFrame:
    """
    ...
    coursework_100_columns = [col for col in df.columns if "Coursework" in col and "(100)" in col]
    if not coursework_100_columns:
        ...
        weighted_columns_present, missing_weighted_columns = check_for_weighted_columns(coursework_100_columns)
        if weighted_columns_present == False:
            raise ValueError(f"""DataFrame is missing weighted columns: {', '.join(missing_weighted_columns)}
    ...
    for col in coursework_100_columns:
        if df[col].dtype not in ["int64", "float64"]:
            raise ValueError(f"""Column {col} is not numeric, you can only calculate the weighted score of numeric columns.
