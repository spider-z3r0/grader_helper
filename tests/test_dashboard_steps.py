...
@pytest.fixture
def group_module(tmp_path):
    """A module whose one assessment is submitted by groups.

    Built here rather than in fake_module because the shape is the point: a
    class list with no group column, a membership the leader keeps, and
    Brightspace's group-format submission folders.
    """
    from openpyxl import Workbook

    from grader_helper.models import init_module

    root = tmp_path / "PS4099"
    root.mkdir()

    students = ["23304301", "23304302", "23304303", "00123456"]
    pd.DataFrame(
        {
            "Username": [f"#{s}" for s in students],
            "Last Name": ["Egan", "Angood", "Joyce", "Lynch"],
            "First Name": ["Aoife", "Ben", "Cara", "Dan"],
        }
    ).to_csv(root / "classlist.csv", index=False)

    # The leader keeps the membership: two teams of two. Written as "Group 1"
    # and "Team 2" on purpose -- leaders are not consistent, and the join has
    # to survive it.
    pd.DataFrame(
        {
            "Student ID": students,
            "Group": ["Group 1", "Group 1", "Team 2", "Team 2"],
        }
    ).to_csv(root / "groups.csv", index=False)
    ...
    init_module(
        ...
        paths={"classlist": "classlist.csv", "groups": "groups.csv"},
    )

    assessment = root / "assessments" / "proj"
    blank = Workbook()
    blank.save(assessment / "Feedback sheet.xlsx")

    # Brightspace's own group-submission format.
    for group in ("Group 1", "Group 2"):
        folder = (
            assessment / "submissions"
            / f"27236-46025 - {group} - 05 March 2026 612 PM"
        )
        folder.mkdir(parents=True)
        (folder / "project.docx").write_bytes(b"")

    return root


def test_a_group_assessment_runs_from_the_buttons(group_module, dashboard):
    """The thing groups are for: one grader per group, one mark per group.

    Nobody should mark half a project, and the mark the group is given has to
    reach every member before the departmental sheet can have a row each.
    """
    names = dashboard(group_module)
    class_list = names["class_list"]
    proj = names["found"].module.assessment("proj")

    # Groups came off the leader's own spreadsheet, not the class list.
    assert class_list is not None, "the groups file should have been picked up"
    assert "Group" in class_list.columns
    assert class_list["Group"].nunique() == 2

    # --- allocate ---------------------------------------------------------
    master, workbooks, allocation = names["allocate_marking"](proj, class_list)

    per_group = allocation.groupby("Group")["grader"].nunique()
    assert per_group.eq(1).all(), (
        f"a group was split across graders: {per_group.to_dict()}"
    )
    assert master.students == len(class_list)

    # --- distribute -------------------------------------------------------
    before = sorted(p.name for p in proj.submissions_path.iterdir() if p.is_dir())
    distribution, log = names["distribute_sheets"](proj, class_list)
    after = sorted(p.name for p in proj.submissions_path.iterdir() if p.is_dir())

    assert sorted(distribution.copied) == ["Group 1", "Group 2"], (
        "one sheet per group, named for the group"
    )
    assert after == before, "a group assessment's folders are left as they are"
    assert log.empty

    # --- the graders mark one sheet per group -----------------------------
    for index, sheet in enumerate(
        sorted(proj.submissions_path.glob("*/Feedback sheet *.xlsx"))
    ):
        workbook = load_workbook(sheet)
        workbook.active[proj.grade_cell] = 60 + index * 5
        workbook.save(sheet)

    # and copy each group's mark onto every member in their own workbook
    marks_by_group = {"Group 1": 60, "Team 2": 65}
    for grader in [g.initials for g in proj.graders]:
        book = proj.grading_output_path / f"{grader}.xlsx"
        allocated = pd.read_excel(book, dtype={"Student ID": str})
        allocated["Mark"] = [marks_by_group[g] for g in allocated["Group"]]
        allocated.to_excel(book, index=False)

    # --- collect ----------------------------------------------------------
    collation, audit = names["collect_marks"](proj, class_list=class_list)

    assert collation.students == len(class_list)
    assert audit.transcription_slips.empty, (
        "the group's mark should reach every member unchanged: "
        f"{audit.transcription_slips}"
    )
    assert audit.agree


def test_renaming_back_is_not_a_group_assessments_problem(group_module, dashboard):
    """Its folders were never renamed, so there is nothing to restore."""
    names = dashboard(group_module)
    proj = names["found"].module.assessment("proj")

    with pytest.raises(ValueError, match="nothing"):
        names["rename_back"](proj)


@pytest.fixture
def group_module_individual_folders(group_module):
    """The other shape: marked by group, submitted one folder per student.

    Which of the two you get is how the leader set the assignment up, so the
    step has to read it off the folders rather than assume.
    """
    submissions = group_module / "assessments" / "proj" / "submissions"
    for folder in list(submissions.iterdir()):
        shutil.rmtree(folder)

    for student, surname in (
        ("23304301", "Egan"),
        ("23304302", "Angood"),
        ("23304303", "Joyce"),
        ("00123456", "Lynch"),
        ...
        folder = (
            submissions
            / f"27236-46025 - {student} {surname} - 05 March 2026 612 PM"
        )
        folder.mkdir(parents=True)
        (folder / "project.docx").write_bytes(b"")
    return group_module
...
def test_a_group_assessments_folders_are_never_renamed(
    group_module_individual_folders, dashboard
):
    """Even where they could be.

    With one folder per student the rename would work — that is exactly why
    it has to be suppressed deliberately. A group assessment's folders go
    back to Brightspace as they came, so nothing renames them and nothing
    has to rename them back.
    """
    names = dashboard(group_module_individual_folders)
    proj = names["found"].module.assessment("proj")
    class_list = names["class_list"]
    ...
    assert after == before, "the folders were renamed"
    assert not (proj.submissions_path / "folder_rename_log.csv").exists()
    assert log.empty
    # One sheet per student here, because that is what was submitted.
    assert sorted(distribution.copied) == sorted(class_list["Student ID"])
